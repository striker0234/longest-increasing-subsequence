...
vi adj[200001];
void dfs1(int root,vi &vis,stack<int>&st,vi adj[]){
    vis[root] = 1;
...
void dfs2(int root,vi &vis,vi adjT[],int sc,vi scc[]){
    vis[root] = 1;
    scc[sc].pb(root);
    for(auto it : adjT[root]){
        if(!vis[it]) dfs2(it, vis, adjT, sc, scc);
    }
}
